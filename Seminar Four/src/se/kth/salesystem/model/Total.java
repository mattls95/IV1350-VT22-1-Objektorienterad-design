...
public class Total {
	...
	public Amount calculateTotalVAT() {
		ArrayList<Item> items = sale.getItems();
		Item item = items.get(items.size() - 1);
		totalVat.multipleAmount(item.getItemDescription().getPrice().getAmount(),
				item.getItemDescription().getVATrate());
		return totalVat;
	 ...
	 * @return totalAmount The total amount
	 */
	public Amount increaseAmount(Item item) {
		totalAmount.increaseAmount(item.getItemDescription().getPrice().getAmount());
		return totalAmount;
	}

	/**
	 * Increases the total VAT.
	 * 
	 * @return totalVat The total Vat
	 */
	public Amount increaseVAT(Item item) {
		totalVat.multipleAmount(item.getItemDescription().getPrice().getAmount(),
				item.getItemDescription().getVATrate());
		return totalVat;
	}
